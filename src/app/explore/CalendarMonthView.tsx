...
// Navigation bounds — calendar is locked to April 2025 … December 2026.
const MIN_YEAR = 2025;
const MIN_MONTH = 3;  // April (0-indexed)
const MAX_YEAR = 2026;
const MAX_MONTH = 11; // December (0-indexed)
const monthIndex = (y: number, m: number) => y * 12 + m;
const MIN_INDEX = monthIndex(MIN_YEAR, MIN_MONTH);
const MAX_INDEX = monthIndex(MAX_YEAR, MAX_MONTH);
...
export function CalendarMonthView({ initialYear = 2026, initialMonth = TODAY_MONTH }: CalendarMonthViewProps) {
  ...
  const atMin = monthIndex(year, month) <= MIN_INDEX;
  const atMax = monthIndex(year, month) >= MAX_INDEX;
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-2">
          <button onClick={goPrev} disabled={atMin} aria-disabled={atMin}
            className="w-7 h-7 rounded-full bg-cream-soft flex items-center justify-center text-ink-muted text-sm disabled:opacity-30 disabled:cursor-not-allowed">
            ‹
          </button>
          <div className="font-serif text-[19px] text-ink font-medium min-w-[120px] text-center">
            {AR_MONTH_NAMES[month]} {toArabicNum(year)}
          </div>
          <button onClick={goNext} disabled={atMax} aria-disabled={atMax}
            className="w-7 h-7 rounded-full bg-cream-soft flex items-center justify-center text-ink-muted text-sm disabled:opacity-30 disabled:cursor-not-allowed">
            ›
          </button>
        </div>
