...
export function PlacementDetailClient({ type, decodedKey }: { type: string; decodedKey: string }) {
  ...
  return (
    <div className="pb-32">
      ...
      {content ? (
        <>
          ...
          <div className="px-5 mt-[14px] flex flex-col gap-4">
            {[
              ['The Configuration', content.traditional],
              ['Soul Development', content.evolutionary],
              ['The Task', content.developmental],
            ].map(([k, t]) => (
              <div key={k}>
          ...
          {houseContent && (
            <div className="px-5 mt-6 flex flex-col gap-4">
              ...
              {[
                ...
                ['The Task', houseContent.developmental],
              ].map(([k, t]) => (
                <div key={k}>
                  <div className="text-[11px] text-ink-muted tracking-wide font-semibold">{k}</div>
                  <div className="flex flex-col gap-2 mt-1.5">
      ...
      <CalibrateBlock storageKey={`hygiea.calibration.${type}:${decodedKey}`} />
